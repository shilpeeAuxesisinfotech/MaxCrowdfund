...
public class PortFolioAdapter extends RecyclerView.Adapter<PortFolioAdapter.MyHolder> {
    ...
    public void onBindViewHolder(@NonNull MyHolder holder, int position) {
        if (position == 0) {
            ...
            holder.tvTittle.setText(arrayList.get(position).getmPortfolioTitle());
            ...
            holder.tvName.setText(arrayList.get(position).getmPortfolioTitle());
            holder.tvName.setTextColor(mContext.getResources().getColor(R.color.light_gray_text));
            holder.tvValue.setText(arrayList.get(position).getmPortfolioValue() + "%");
        }
    }
